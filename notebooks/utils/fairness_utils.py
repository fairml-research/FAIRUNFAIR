...
def get_subroups_results_OLD(X, y, S, y_pred, 
    ...
    df3 = df3[df3["ytrue"]>=n_min].reset_index().rename(columns={"ytrue": "n_obs", "Yhat": "Local DI", "acc":"Acc by S", "acc2":"Local Acc", "SubGroup":"group_label"}) #filter by size of segments and rename columns
    ...
    df3["SubGroup"]= df3["group_label"] # arrondi 3 chiffres
...
def get_subroups_results(df_groups, y, S, y_pred, 
    ...
    print("coucou")
    df3 = df3[df3["ytrue"]>=n_min].reset_index().rename(columns={"ytrue": "n_obs", "Yhat": "Local DI", "acc":"Acc by S", "acc2":"Local Acc", "sex33":"sex"}) #segments and rename columns
    df3["Local Acc"] = df3["Local Acc"] / df3["n_obs"]
    df3["Local Acc"] = df3["Local Acc"].round(3)
    df3["Local DI"]= df3["Local DI"].round(3) # arrondi 3 chiffres
    #df3["SubGroup"]= df3["group_label"] # arrondi 3 chiffres
    DATAGROUPE = df_groups.drop(["sensitive"], axis=1).drop_duplicates().sort_values(by='group_label')
    df4 = df3.merge(DATAGROUPE, on='group_label', how='left')
...
def get_subroups_results_EO(df_groups, y, S, y_pred, 
    ...
    #df_results2["Yhat2"] = df_results2["Yhat2"].round(3).reset_index()
    #df_results2["Yhat3"] = df_results2["Yhat3"].round(3).reset_index()
    #df_results2 = df_results2[df_results2[]]
    df_results2=df_results2.reset_index()
    #print(df_results2.reset_index())
    ...
    #df3 = df_results2.reset_index().groupby("group_label""ytrue").agg({"Yhat3":lambda x: x.max() - x.min()}) 
    #aggregate genders and get: size(seg), |E(^Y|S=1)-E(^Y|S=0)|, [acc(f|seg0), acc(f|seg1)
    #df3b = df4[df4['ytrue'] ==0]
    #print(df3b)
    #df3b = df4["ytrue"].filter({"ytrue":lambda x: x==1})
    #print(df3b)
    #df3 = df4["Yhat3"][df4["ytrue"]==1]
    
    #"acc3":"Local FNR", "acc4":"Local FPR"
    ...
    #df3["Local FNR"]= df3["Local FNR"].round(3) # arrondi 3 chiffres
    #df3["Local FPR"]= df3["Local FPR"].round(3) # arrondi 3 chiffres
