...
class ZHANG(object):
    ...
    def __init__(self, learning_rate, batch_size, lamb,
                 num_epochs, NN_y, NN_s, GPU, name="01"):
        ...
        self.name = "zhang_" + str(lamb) + "_" + name
    ...
    def fit(self, X_train, y_train, S_train, X_test=None, y_test=None, S_test=None, plot_losses=False):
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch % 5 == 0:
                y_pred2= self.m_NN_y(torch.FloatTensor(X_train).to(self.device)).cpu().data.numpy()
                if X_test is not None:
                    y_pred2t= self.m_NN_y(torch.FloatTensor(X_test).to(self.device)).cpu().data.numpy()
                    print('epoch', epoch, 'loss', loss.cpu().data.numpy(), 'lossS', lossS.cpu().data.numpy(), 'lossY', lossY.cpu().data.numpy(),'P-rule', 'ACC_train',accuracy_score(y_train, y_pred2>0.5))
                else: 
                    print('epoch', epoch, 'loss', loss.cpu().data.numpy(), 'lossS', lossS.cpu().data.numpy(), 'lossY', lossY.cpu().data.numpy())
    ...
    def predict(self, X, threshold=0.5):
        return self.m_NN_y(torch.FloatTensor(X).to(self.device)).cpu().data.numpy()
