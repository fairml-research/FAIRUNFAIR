...
class GRARI(torch.nn.Module): 
    ...
    def predict_after_proba(self, X, threshold=0.5):
        return (torch.sigmoid(self.model_h(torch.FloatTensor(X).to(self.device))).cpu().data.numpy()>threshold).astype('int')
    def predict_proba(self, X):
        return torch.sigmoid(self.model_h(torch.FloatTensor(X).to(self.device))).cpu().data.numpy()

    def predict(self, X_train): 
        x_var = Variable(torch.FloatTensor(X_train)).to(self.device)
